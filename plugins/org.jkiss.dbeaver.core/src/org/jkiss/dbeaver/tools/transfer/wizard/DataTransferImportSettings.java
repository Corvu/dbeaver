...
import org.jkiss.dbeaver.registry.transfer.DataTransferImportRegistry;
 ...
 */
public class DataTransferImportSettings {

    private static final Log log = Log.getLog(DataTransferImportSettings.class);
    ...
    public DataTransferImportSettings(@Nullable IDataTransferProducer[] producers, @Nullable IDataTransferConsumer[] consumers)
    {
        ...
        if (!ArrayUtils.isEmpty(producers) && !ArrayUtils.isEmpty(consumers)) {
            ...
            DataTransferNodeDescriptor producerDesc = DataTransferImportRegistry.getInstance().getNodeByType(producerType);
            ...
            DataTransferNodeDescriptor consumerDesc = DataTransferImportRegistry.getInstance().getNodeByType(consumerType);
        ...
        DataTransferImportRegistry registry = DataTransferImportRegistry.getInstance();
    ...
    void addWizardPages(DataTransferImportWizard wizard)
    {
    ...
    void loadFrom(IRunnableContext runnableContext, IDialogSettings dialogSettings)
    {
        ...
        if (!CommonUtils.isEmpty(producerId)) {
            DataTransferNodeDescriptor producerNode = DataTransferImportRegistry.getInstance().getNodeById(producerId);
        ...
        if (consumerOptional) {
            ...
            if (!CommonUtils.isEmpty(consumerId)) {
                DataTransferNodeDescriptor consumerNode = DataTransferImportRegistry.getInstance().getNodeById(consumerId);
        ...
        if (processorsSection != null) {
            for (IDialogSettings procSection : ArrayUtils.safeArray(processorsSection.getSections())) {
                ...
                DataTransferNodeDescriptor node = DataTransferImportRegistry.getInstance().getNodeById(nodeId);
                if (node != null) {
